...
async function startUpdate() {
  ...
  try {
    ...
    if (updateMode.value === 'file') {
      ...
      if (!downloadRes.ok || downloadRes.data?.error) throw new Error(downloadRes.data?.error || 'Download failed')
      uploadProgress.value = 100
    ...
    if (!extractRes.ok || extractRes.data?.error) throw new Error(extractRes.data?.error || 'Extract failed')
    ...
    if (!installRes.ok || installRes.data?.error) throw new Error(installRes.data?.error || 'Install failed')

    installProgress.value = 100
    addLog('✓ ' + t('update.installComplete'))
    if (installRes.data?.output) addLog(t('update.output') + ': ' + installRes.data.output)
