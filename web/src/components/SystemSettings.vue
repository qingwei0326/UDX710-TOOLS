...
// 设备类型
const deviceType = ref(localStorage.getItem('device_type') || 'battery')
...
const menuIcons = {
  ...
  battery: 'fa-battery-half', rathole: 'fa-shield-alt', ipv6proxy: 'fa-network-wired',
  update: 'fa-cloud-download-alt', at: 'fa-terminal', terminal: 'fa-desktop',
  usb: ['fab', 'usb'], plugins: 'fa-puzzle-piece'
}
const menuHidden = ref(JSON.parse(localStorage.getItem('menu_hidden') || '[]'))

function toggleMenuItem(id) {
  const idx = menuHidden.value.indexOf(id)
  if (idx >= 0) {
    menuHidden.value.splice(idx, 1)
  } else {
    menuHidden.value.push(id)
  }
  localStorage.setItem('menu_hidden', JSON.stringify(menuHidden.value))
...
function startLocalTimeTick() {
  if (timeTickInterval) clearInterval(timeTickInterval)
  timeTickInterval = setInterval(() => {
    if (!currentTime.value) return
    // 解析 "2025-01-01 12:00:00" 格式并加1秒
    const d = new Date(currentTime.value.replace(' ', 'T'))
...
onMounted(() => {
  fetchRebootConfig()
  fetchSystemTime().then(() => startLocalTimeTick())
...
<template>
  <div class="space-y-4 sm:space-y-6">
    ...
    <div class="rounded-2xl bg-white/95 dark:bg-white/5 backdrop-blur border border-slate-200/60 dark:border-white/10 p-6 shadow-lg shadow-slate-200/40 dark:shadow-black/20 hover:shadow-xl hover:shadow-slate-300/50 dark:hover:shadow-black/30 transition-all duration-300">
      ...
      <!-- 设备类型 -->
      <div class="mt-6 flex items-center justify-between p-4 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10">
        <div class="flex items-center space-x-3">
          <div class="w-10 h-10 rounded-lg bg-amber-500/20 flex items-center justify-center">
            <i class="fas fa-microchip text-amber-500 dark:text-amber-400"></i>
          </div>
          <div>
            <p class="text-slate-900 dark:text-white font-medium">{{ t('settings.deviceType') }}</p>
            <p class="text-slate-500 dark:text-white/50 text-sm">{{ t('settings.deviceTypeDesc') }}</p>
          </div>
        </div>
        <div class="flex bg-slate-200 dark:bg-white/10 rounded-lg p-1">
          <button
            @click="setDeviceType('battery')"
            class="px-3 py-1.5 rounded-md text-sm font-medium transition-all"
            :class="deviceType === 'battery' ? 'bg-amber-500 text-white shadow-sm' : 'text-slate-600 dark:text-white/60 hover:text-slate-900 dark:hover:text-white'"
          >{{ t('settings.withBattery') }}</button>
          <button
            @click="setDeviceType('no-battery')"
            class="px-3 py-1.5 rounded-md text-sm font-medium transition-all"
            :class="deviceType === 'no-battery' ? 'bg-amber-500 text-white shadow-sm' : 'text-slate-600 dark:text-white/60 hover:text-slate-900 dark:hover:text-white'"
          >{{ t('settings.withoutBattery') }}</button>
        </div>
      </div>

      <!-- 菜单管理 -->
      <div class="mt-6 p-4 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10">
        <div class="flex items-center space-x-3 mb-4">
          <div class="w-8 h-8 rounded-lg bg-violet-500/20 flex items-center justify-center">
            <i class="fas fa-list-check text-violet-500 dark:text-violet-400 text-sm"></i>
          </div>
          <p class="text-slate-900 dark:text-white font-medium text-sm">{{ t('settings.menuManage') }}</p>
        </div>
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
          <button
            v-for="key in allMenuKeys"
            :key="key"
            @click="toggleMenuItem(key)"
            class="flex items-center space-x-2 px-3 py-2 rounded-lg text-xs font-medium transition-all border"
            :class="menuHidden.includes(key)
          ...
          >
            <font-awesome-icon :icon="menuIcons[key]" class="text-sm" />
            <span :class="menuHidden.includes(key) ? 'line-through' : ''">{{ t('menu.' + key) }}</span>
          </button>
        </div>
        <p class="text-slate-400 dark:text-white/30 text-xs mt-3">{{ t('settings.menuManageDesc') }}</p>
      </div>
    </div>
