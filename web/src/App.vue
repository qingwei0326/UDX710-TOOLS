...
async function handleLogin() {
  ...
  try {
    ...
    if (result.ok && result.data.token) {
      ...
      // token已由authLogin存入localStorage，无需保存明文密码
...
const menuItems = computed(() => {
  return allMenuItems.filter(item => {
    if (item.id === 'settings') return true // 设置始终显示
    if (deviceType.value === 'no-battery' && item.id === 'battery') return false
    if (menuHidden.value.includes(item.id)) return false
    return true
  })
})
          ...
          <div class="flex items-center justify-between gap-2 md:gap-4">
            ...
            <div class="flex items-center space-x-2 md:space-x-4 flex-shrink-0">
              ...
              <button
                ...
                class="w-9 h-9 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-slate-200 dark:bg-white/10 hover:bg-slate-300 dark:hover:bg-white/20 transition-colors flex items-center justify-center"
                :title="deviceType === 'battery' ? t('settings.withBattery') : t('settings.withoutBattery')"
              >
                <font-awesome-icon
                  :icon="deviceType === 'battery' ? 'battery-full' : 'battery-empty'"
                  class="text-sm md:text-base"
                  :class="deviceType === 'battery' ? 'text-green-500 dark:text-green-400' : 'text-slate-400 dark:text-white/40'"
                />
              </button>
              <!-- 主题切换按钮 -->
              <button 
